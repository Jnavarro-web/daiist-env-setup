...
def pandas_revenue_by_region(sales: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    sales_with_revenue = sales.assign(revenue=sales["quantity"] * sales["unit_price"])
    return (
        sales_with_revenue.groupby("region", as_index=False)["revenue"]
        .sum()
    )
...
def pandas_region_share(sales: pd.DataFrame) -> pd.DataFrame:
    ...
    grand_total = pd.DataFrame({"total_revenue": [revenue_by_region["revenue"].sum()]})
